...
impl Logger {
    ...
    pub fn run(&mut self) {
        ...
        while !stop {
            let text = self.log_receive.recv().expect("Error receiving msg in Logger");

            if text == "STOP" {
                stop = true;
            } else {
                self.file.write(text.as_ref()).expect("Error writing to logger file");
            }
        }
    }
}
...
pub fn log_info(msg: String, log_send: Sender<String>) {
    ...
    log_send.send(log_info).expect("Error sending info msg to logger");
}
...
pub fn log_stop(log_send: Sender<String>, logger_handle: JoinHandle<()>) {
    let msg = format!("STOP");
    log_send.send(msg).expect("Error sending STOP msg to logger");
